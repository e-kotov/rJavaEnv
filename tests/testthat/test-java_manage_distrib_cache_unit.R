...
test_that("java_list_distrib returns empty when no distrib folder exists", {
  local_cache <- withr::local_tempdir()
  withr::local_options(rJavaEnv.cache_path = local_cache)

  # No "distrib" folder created
  result <- java_list_distrib(cache_path = local_cache)

  expect_equal(result, character(0))
})

test_that("java_list_distrib returns data.frame with files", {
  local_cache <- withr::local_tempdir()
...
test_that("java_list_distrib returns vector when requested", {
  local_cache <- withr::local_tempdir()
  dist_dir <- file.path(local_cache, "distrib")
  dir.create(dist_dir, recursive = TRUE)
  ...
  result <- java_list_distrib(cache_path = local_cache, output = "vector")
