...
The default install path is into **rjavaenv/`platform`/`processor_architecture`/`major_java_version`** folder in the current working/project directory, but can be customised, see docs for `rJavaEnv::java_install()`.
...
The limitation is that if you want to switch to another Java environment, you will have to restart the current R session and set the JAVA_HOME and PATH environment variables to the desired Java environment from scratch using `rJavaEnv::java_set_env()`.
