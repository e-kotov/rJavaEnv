...
Assume your project directory is currently in a temporary directory. Feel free to skip that, if you are already working in a desired project directory wehre you would like to install `Java`. In the example outputs below you will see paths that point to a temporary directory, but in a real project you would see your project directory instead.
...
`java_download()` also allows you to define the operating system and processor architecture with function arguments `platform` and `arch` (pleasee see the function documentation using `?rJavaEnv::java_download`). For example, this may be useful if you are packaging a Java distribtution for Linux while using a Windows or macOS machine. This may also be useful if for some reason automatic OS and CPU detection fails.
...
You can manage downloaded distribtuions with the `java_list` and `java_clear` functions.
...
[^1]: In Windows, it is sometimes impossible to create junctions for files that are located on different drives. Foe example, if `rJavaEnv` is installed on a `C:` drive, and the `Java` distribution cache is in the same drive in user folder (which it isually is), the Java home folder cannot be linked using junctions into a project folder that is, for example, on drive `D:`. In this case, the `Java` installation will have to be copied into the project folder, which will take some time. This behaviour is the same `renv`'s for R packages and is an operating system limitation.
...
```
Java distribution amazon-corretto-8-x64-windows-jdk.zip already unpacked at C:\Users\kotov\AppData\Local/R/cache/R/rJavaEnv/installed/windows/x64/8
v Current R Session: JAVA_HOME and PATH set to C:\Users\kotov\AppData\Local/R/cache/R/rJavaEnv/installed/windows/x64/8
v Current R Project/Working Directory: JAVA_HOME and PATH set to 'C:\Users\kotov\AppData\Local/R/cache/R/rJavaEnv/installed/windows/x64/8' in .Rprofile at 'C:/Users/kotov/AppData/Local/Temp/75/R
tmp0MasTW'
Java 8 (amazon-corretto-8-x64-windows-jdk.zip) for windows x64 installed at C:\Users\kotov\AppData\Local/R/cache/R/rJavaEnv/installed/windows/x64/8 and symlinked to
C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/8
```
...
```
JAVA_HOME: C:\Users\kotov\AppData\Local/R/cache/R/rJavaEnv/installed/windows/x64/8
Java path: /c/Users/kotov/AppData/Local/R/cache/R/rJavaEnv/installed/windows/x64/8/bin/java
...
```
path platform arch version
1 C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17  windows  x64      17
2  C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/8  windows  x64       8
```
...
```
"C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17"
```
...
```
v Current R Session: JAVA_HOME and PATH set to C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17
v Current R Project/Working Directory: JAVA_HOME and PATH set to 'C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17' in .Rprofile at 'C:/Users/kotov/AppData/Local/Temp/75/R
tmp0MasTW'
```
...
```
JAVA_HOME: C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17
Java path: /c/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17/bin/java
...
Using current session's JAVA_HOME: C:/Users/kotov/AppData/Local/Temp/75/Rtmp0MasTW/rjavaenv/windows/x64/17
